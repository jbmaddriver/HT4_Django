...
def gen_student(request):
    student = Student.objects.create(
        ...
        age=fake.random_int(min=18, max=45, step=1),
    )
...
# Generate users using Faker with query parameters from URL link. Example: PATH....generate-students/?num=15
def gen_students100(request):
    ...
    if 0 < query_number <= 100:
        ...
        for s in range(gen_number):
            temp_arg = Student.objects.create(
                first_name=fake.first_name(),
                last_name=fake.last_name(),
                age=fake.random_int(min=18, max=45, step=1),
            )
        ...
        return HttpResponse(
            f'Wrong number, please enter "num" integer value between 1 and 100. See example: ".../?num=100"'
        )
